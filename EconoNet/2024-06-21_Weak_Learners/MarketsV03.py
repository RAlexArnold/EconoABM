...
class SCDA:
    ...
    def __init__(self, Nproducts, theta=0.1, gamma=1.0, epsilon=0.005, max_tries=100, M_min=0.01, peval='log'):
    ...
    def update_prices(self, i):
        ...
        self.agent_data[self.successful_bids, 0, i]   *= (1.0 - delta_bidding_price[self.successful_bids])
    
        # For uncessful bids, the buyers will increase their future bids to try to get a deal
        self.agent_data[self.unsuccessful_bids, 0, i] *= (1.0 + delta_bidding_price[self.unsuccessful_bids])
        ...
        self.agent_data[self.successful_asks, 1, i] *= (1.0 + delta_asking_price[self.successful_asks])
    
        # For unsuccessful asks, the sellers will decrease their future asks to try to get a deal
        self.agent_data[self.unsuccessful_asks, 1, i] *= (1.0 - delta_asking_price[self.unsuccessful_asks])
    ...
    def attempt_trial(self):
        ...
        if (N_buyers == 0) or (N_sellers == 0):
            ...
            try:
                ...
                print(self.sellers_mask)
                print(self.exchanging_agent_list[self.sellers_mask])
                print(softmax_asks)
                print(N_transacting_agents)
                assert False

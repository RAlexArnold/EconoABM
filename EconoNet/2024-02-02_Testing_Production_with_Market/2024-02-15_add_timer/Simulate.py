...
class Simulate():
    ...
    def __init__(self, env): #agent_list, inst_list, dist_list, env):
        ...
        self.reward_trigger_learn = None
    ...
    def set_reward_trigger_learn(self, reward_trigger_learn):
        
        # Agent will only learn if their reward is less than the value of `reward_trigger_learn`
        # If None, then agent will always learn
        self.reward_trigger_learn = reward_trigger_learn
    ...
    def get_state(self, agent):
        ...
        Xi_column_sums = agent.Ins.prod_vec
    
        observation = np.concatenate([agent.Q, agent.D, [agent.M], Xi_column_sums, [self.env.time]])
    ...
    def update_market_arrays(self, ti):
        ...
        self.mMarray[ti] = self.market.money_exchanged
    ...
    def run_simulation(self, Ntimes):
        ...
        #    raise ValueError('Reward Function not defined. Define using set_reward_function()')
        # defaults to deficit
        ...
        self.epsilon_list = [] # Same across all agents
    ...
    def sim_step(self, ti):
        ...
        self.epsilon_list.append(self.agent_list[0].epsilon) # all agents share same epsilon, so just use first agent's
        ...
        for agent in self.agent_list:
            ...
            # Agent should learn if the reward is less than some value
            if (self.reward_trigger_learn is not None) and (reward < self.reward_trigger_learn):
                agent.learn()
            else:
                agent.learn()

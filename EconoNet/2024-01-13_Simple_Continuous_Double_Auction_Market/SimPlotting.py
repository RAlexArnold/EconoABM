# -*- coding: utf-8 -*-
"""
Created on Sat Nov  4 02:53:14 2023
...
class SimPlotting:
    ...
    def update(self):
        ...
        self.StockArray = self.sim.StockArray
        self.DepositsArray = self.sim.DepositsArray
        self.WithdrawalsArray = self.sim.WithdrawalsArray
    ...
    def plotStock(self, ax, lw1, lw2):
        
        stock = self.StockArray
        t0 = self.dt/2.0
        
        [ax.plot(self.trange + t0, stock[:,i], label='rf$S_{i}$', lw=lw1) for i in range(self.Nproducts)]
        
        ax.set_xlabel('$t$')
        ax.set_ylabel('Quantities')
        
    def plotDeposits(self, ax, lw1, lw2):
        
        deposit = self.DepositsArray
        t0 = self.dt/2.0
        
        [ax.plot(self.trange + t0, -deposit[:,i], label='rf$+S_{i}$', lw=lw1) for i in range(self.Nproducts)]
        
        ax.set_xlabel('$t$')
        ax.set_ylabel('Deposits')
        
    def plotWithdrawals(self, ax, lw1, lw2):
        
        withdrawals = self.WithdrawalsArray
        t0 = self.dt/2.0
        
        [ax.plot(self.trange + t0, withdrawals[:,i], label='rf$-S_{i}$', lw=lw1) for i in range(self.Nproducts)]
        
        ax.set_xlabel('$t$')
        ax.set_ylabel('Withdrawals')
            
    def plotProductBank(self, *, mw=4, mh=8, Slog=False, Dlog=False, Wlog=False, Nlog=False, tight_layout=True, **kwargs):
        
        fig, axs = plt.subplots(4, figsize=(mw, mh))
        
        self.plotStock(axs[0], **kwargs)
        self.plotDeposits(axs[1], **kwargs)
        self.plotWithdrawals(axs[2], **kwargs)
        self.plotmN(axs[3], **kwargs)
